...
#' @param min_bins Integer. Minimum number of bins to produce. Must be >= 1.
...
#'
#' @return A list containing the multinomial binning results with the following components:
#'   \describe{
...
#'
#' @export
ob_categorical_jedi_mwoe <- function(feature, target,
                                     min_bins = 3,
                                     max_bins = 5,
                                     bin_cutoff = 0.05,
                                     max_n_prebins = 20,
  ...
  if (!identical(unique_targets, 0:(length(unique_targets) - 1))) {
    stop("Target must contain consecutive integers starting from 0")
  }

  # Invoke C++ implementation
  .Call("_OptimalBinningWoE_optimal_binning_categorical_jedi_mwoe",
    target = target,
    feature = feature,
