...
#' @param auto_monotonicity Logical. If \code{TRUE}, the algorithm automatically
...
#' @param convergence_threshold Numeric. Reserved for future use. Currently not
#'   actively used by the PAVA algorithm, which has guaranteed convergence.
#'   Defaults to 1e-6.
#' @param max_iterations Integer. Safety limit for iterative merging operations
...
#'
#' @seealso \code{\link{ob_numerical_dp}} for greedy dynamic programming approaches.
#'
#' @examples
...
#'
#' @export
ob_numerical_ir <- function(feature, target, min_bins = 3, max_bins = 5,
                            bin_cutoff = 0.05, max_n_prebins = 20,
                            auto_monotonicity = TRUE, convergence_threshold = 1e-6,
                            max_iterations = 1000) {
  ...
  #  auto_monotonicity, convergence_threshold, max_iterations)
  .Call("_OptimalBinningWoE_optimal_binning_numerical_ir",
    target,
    feature,
    as.integer(min_bins),
    as.integer(max_bins),
