...
#' @param max_n_prebins Integer. Maximum number of initial bins/groups allowed
...
#' @param alpha Numeric. Laplace smoothing parameter added to counts to avoid
...
#'
#' @export
ob_categorical_sblp <- function(feature,
                                target,
                                min_bins = 3L,
                                max_bins = 5L,
                                bin_cutoff = 0.05,
  ...
  # Convert NA values to "NA" string to ensure they are handled as a category
  ...
  .Call("_OptimalBinningWoE_optimal_binning_categorical_sblp",
    target = target,
    feature = feature,
