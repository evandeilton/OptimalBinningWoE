...
fitted.oblr <- function(object, ...) {
  return(predict(object, type = "response"))
}
...
#' @title Stepwise Model Selection for oblr Objects
#' @description
#' Performs stepwise model selection (backward, forward, both) for `oblr` model objects
#' based on the Akaike Information Criterion (AIC).
#'
#' @details
#' The `step.oblr` function iteratively adds or removes predictors from an `oblr` model
#' to optimize the AIC. It supports three directions of selection:
#'
#' - **Backward**: Starts with the full model and removes predictors.
#' - **Forward**: Starts with a minimal model and adds predictors.
#' - **Both**: Combines both backward elimination and forward selection.
#'
...
#'
#' @param object An object of class `oblr`.
#' @param scope A two-sided formula defining the range of models examined in the stepwise search.
#'        Can be a single formula or a list with `lower` and `upper` components.
#'        If missing, the scope is determined based on the current model.
#' @param scale Unused. Set to `0` by default.
#' @param direction The direction of stepwise search. Must be one of `"both"`, `"backward"`, or `"forward"`.
#' @param trace An integer indicating the level of tracing information to be printed. `0` for no tracing,
#'        `1` for minimal tracing, and higher values for more detailed tracing.
#' @param keep An optional function to be applied to each model as it is evaluated.
#'        Useful for tracking model selection history. Set to `NULL` by default.
#' @param steps The maximum number of steps to perform. Default is `1000`.
#' @param k The penalty per parameter to be used in the AIC calculation. Default is `2`.
#' @param ... Additional arguments passed to or from other methods.
#'
#' @return An `oblr` object representing the final selected model after stepwise selection.
#'         Attributes include the selection path and AIC history if `keep` is utilized.
#'
#' @examples
#' \dontrun{
...
#'
#' @export
step.oblr <- function(object, scope = NULL, scale = 0,
                      direction = c("both", "backward", "forward"),
                      trace = 1, keep = NULL, steps = 1000, k = 2, ...) {
  # Match the direction argument
  direction <- match.arg(direction)

  # Extract the original call to retrieve formula and data
  original_call <- object$data$call

  # Retrieve the formula and data from the original call
  formula_full <- eval(original_call$formula)
  data_full <- eval(original_call$data)

  # Ensure that the data is a data frame
  if (!is.data.frame(data_full)) {
    stop("The data provided to the oblr model must be a data frame.")
  }

  # Determine the scope if not provided
  if (is.null(scope)) {
    scope <- formula_full
  }
  ...
  if (inherits(scope, "formula")) {
    # Single formula provided
    scope_list <- list(
      lower = update.formula(scope, . ~ 1),
      upper = scope
    )
  } else if (is.list(scope) && all(c("lower", "upper") %in% names(scope))) {
    # List with lower and upper formulas
    scope_list <- scope
  } else {
    stop("Scope must be a formula or a list containing 'lower' and 'upper' formulas.")
  }

  # Extract terms from the upper and lower scope
  upper_terms <- attr(terms(scope_list$upper), "term.labels")
  lower_terms <- attr(terms(scope_list$lower), "term.labels")

  # All possible predictors within the scope
  all_predictors <- unique(c(upper_terms, lower_terms))

  # Current model's predictors
  current_terms <- attr(terms(formula_full), "term.labels")

  # Define the minimal model based on the lower scope
  formula_min <- scope_list$lower

  # Initialize stepwise selection
  current_model <- object
  current_aic <- -2 * current_model$loglikelihood + k * length(current_model$coefficients)
  if (trace >= 1) {
    cat("Stepwise selection start:\n")
    cat("Initial model:", deparse(formula_full), "\n")
    cat("Initial AIC:", current_aic, "\n")
  }

  # Initialize variables for tracking
  selected_terms <- current_terms
  remaining_terms <- setdiff(all_predictors, selected_terms)

  # For forward selection, if starting from a minimal model
  if (direction == "forward") {
    selected_terms <- lower_terms
    remaining_terms <- setdiff(all_predictors, selected_terms)
    # Fit the minimal model
    current_model <- tryCatch(
      {
        oblr(formula_min, data = data_full)
      },
      error = function(e) {
        stop("Failed to fit the minimal model: ", e$message)
      }
    )
    current_aic <- -2 * current_model$loglikelihood + k * length(current_model$coefficients)
    if (trace >= 1) {
      cat("Starting forward selection with model:", deparse(formula_min), "\n")
      cat("Initial AIC:", current_aic, "\n")
    }
  }
  ...
  # Initialize a list to keep track of the selection path if keep is provided
  if (!is.null(keep)) {
    keep_list <- list()
    keep_list[[1]] <- current_model
  }

  # Start the stepwise loop
  while (improved && step_count < steps) {
    ...
    # Candidates for adding and/or removing
    add_candidates <- if (direction %in% c("forward", "both")) remaining_terms else character(0)
    remove_candidates <- if (direction %in% c("backward", "both")) selected_terms else character(0)

    # List to store candidate models
    ...
    if (length(add_candidates) > 0) {
      for (term in add_candidates) {
        new_terms <- c(selected_terms, term)
        formula_new <- as.formula(paste(deparse(formula_min), "~", paste(new_terms, collapse = " + ")))
        # Fit the new model
        model_new <- tryCatch(
          {
            oblr(formula_new, data = data_full)
          },
          error = function(e) {
            if (trace >= 2) {
              cat("  Failed to add term", term, ":", e$message, "\n")
            }
            NULL
          }
        )
        if (!is.null(model_new)) {
          aic_new <- -2 * model_new$loglikelihood + k * length(model_new$coefficients)
          candidate_models[[term]] <- model_new
          candidate_aics <- c(candidate_aics, aic_new)
          candidate_actions <- c(candidate_actions, "add")
          candidate_terms <- c(candidate_terms, term)
        }
      }
    }
    ...
    if (length(remove_candidates) > 0) {
      for (term in remove_candidates) {
        new_terms <- setdiff(selected_terms, term)
        if (length(new_terms) == 0) {
          formula_new <- update(formula_min, . ~ 1)
        } else {
          formula_new <- as.formula(paste(deparse(formula_min), "~", paste(new_terms, collapse = " + ")))
        }
        ...
        model_new <- tryCatch(
          ...
          error = function(e) {
            if (trace >= 2) {
              cat("  Failed to remove term", term, ":", e$message, "\n")
            }
            NULL
          }
        )
        if (!is.null(model_new)) {
          ...
          candidate_models[[term]] <- model_new
          candidate_aics <- c(candidate_aics, aic_new)
          candidate_actions <- c(candidate_actions, "remove")
          candidate_terms <- c(candidate_terms, term)
        }
      }
    }

    # Check if any candidates were evaluated
    if (length(candidate_aics) == 0) {
      break
    }

    # Find the best candidate
    min_aic <- min(candidate_aics, na.rm = TRUE)
    min_index <- which.min(candidate_aics)
    min_action <- candidate_actions[min_index]
    min_term <- candidate_terms[min_index]
    min_model <- candidate_models[[min_term]]

    # Compare AICs
    if (min_aic < best_aic - 1e-8) { # Use a small threshold to prevent floating point issues
      # Accept the candidate model
      improved <- TRUE
      best_aic <- min_aic
      ...
      # Update selected and remaining terms
      if (best_action == "add") {
        selected_terms <- c(selected_terms, best_term)
        remaining_terms <- setdiff(remaining_terms, best_term)
      } else if (best_action == "remove") {
        selected_terms <- setdiff(selected_terms, best_term)
        remaining_terms <- c(remaining_terms, best_term)
      }
      ...
      # Store the model if keep is provided
      if (!is.null(keep)) {
        keep_list[[step_count + 1]] <- current_model
      }
      ...
      if (trace >= 1) {
        cat(sprintf(
          "Step %d: %s '%s', AIC = %.3f\n",
          step_count,
          ifelse(best_action == "add", "Add", "Remove"),
          best_term,
          best_aic
        ))
      }
    } else {
      # No improvement, stop the stepwise search
      if (trace >= 1) {
        cat("No further improvement in AIC. Stepwise selection terminated.\n")
      }
      break
    }
  }

  # Attach the selection path if keep is used
  if (!is.null(keep)) {
    attr(current_model, "selection_path") <- keep_list
  }

  # Final trace output
  if (trace >= 1) {
    cat(sprintf("Stepwise selection completed in %d steps.\n", step_count))
...
step.default <- function(object, ...) {
  stop("The 'step' function is only implemented for 'oblr' objects.")
}
