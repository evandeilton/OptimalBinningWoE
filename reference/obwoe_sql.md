...
- digits:
  ...
  at full precision, as a decimal string that parses back to the
  identical double. Supplying a value rounds the literals to that many
  decimal places for readability, at the cost of exactness on bin
  boundaries.
...
- bin_separator:
  ...
  When the argument is not supplied and `obj` records the separator it
  was fitted with (an `"obwoe"` object, or a step or recipe whose
  [`step_obwoe()`](https://evandeilton.github.io/OptimalBinningWoE/reference/step_obwoe.md)
  control sets one), that separator is used instead.
...
With `digits = NULL` each cut point is written with the seventeen
significant digits that identify an IEEE 754 double uniquely, trailing
zeros removed, so a value that is exact in binary still reads short.
Rounding a boundary such as `4049.5` to fewer digits would silently move
observations between bins, so exactness is the default; scientific
notation is avoided because dialects differ on how they type such
literals.
