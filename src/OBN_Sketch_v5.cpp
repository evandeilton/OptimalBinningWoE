...
// Estrutura NumericalBin para variáveis numéricas
// struct NumericalBin {
//   double lower_bound;
//   double upper_bound;
//   int count;
...
//   double event_rate;
//   
//   NumericalBin(double lb = -std::numeric_limits<double>::infinity(), 
...
private:
  ...
  void create_initial_bins() {
    ...
    NumericalBin first_bin(min_val, cutpoints[0]);
    bins.push_back(first_bin);
    
    // Bins intermediários
    for (size_t i = 0; i < cutpoints.size() - 1; ++i) {
      NumericalBin bin(cutpoints[i], cutpoints[i + 1]);
      bins.push_back(bin);
    }
    
    // Último bin (do último cutpoint até +inf)
    NumericalBin last_bin(cutpoints.back(), max_val);
...
public:
  ...
  sketch_k(sketch_k_),
  dp_size_limit(50), // Very conservative limit - DP has bugs with larger datasets
  total_good(0), total_bad(0) {
    // Pré-alocação
