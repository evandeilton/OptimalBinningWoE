...
private:
  ...
  void validate_inputs() {
    if (feature.empty()) Rcpp::stop("Feature vector is empty.");
    if (feature.size() != target.size()) Rcpp::stop("Feature and target vectors must have the same length.");
    if (min_bins < 2) Rcpp::stop("min_bins must be >= 2.");
    if (max_bins < min_bins) Rcpp::stop("max_bins must be >= min_bins.");
    if (bin_cutoff < 0 || bin_cutoff > 0.5) Rcpp::stop("bin_cutoff must be between 0 and 0.5.");
    if (max_n_prebins < min_bins) Rcpp::stop("max_n_prebins must be >= min_bins.");
    if (max_n_prebins > 1000) Rcpp::stop("max_n_prebins must be <= 1000.");
  }
  
  void prebin_data() {
    std::vector<std::pair<double, int>> feature_target_pairs;
    feature_target_pairs.reserve(feature.size());
    for (size_t i = 0; i < feature.size(); ++i) {
      if (std::isfinite(feature[i])) {
        feature_target_pairs.emplace_back(feature[i], target[i]);
      }
    }
    
    std::sort(feature_target_pairs.begin(), feature_target_pairs.end());
    size_t n = feature_target_pairs.size();
    
    bin_edges.resize(max_n_prebins + 1);
    for (int i = 0; i <= max_n_prebins; ++i) {
      size_t idx = i * (n - 1) / max_n_prebins;
      bin_edges[i] = feature_target_pairs[idx].first;
    }
    
    bin_edges.front() = std::nextafter(bin_edges.front(), -std::numeric_limits<double>::infinity());
    bin_edges.back() = std::nextafter(bin_edges.back(), std::numeric_limits<double>::infinity());
    
    int n_bins = bin_edges.size() - 1;
    bin_count.assign(n_bins, 0);
    bin_pos_count.assign(n_bins, 0);
    bin_neg_count.assign(n_bins, 0);
    
    for (const auto& pair : feature_target_pairs) {
      int bin_idx = std::upper_bound(bin_edges.begin(), bin_edges.end(), pair.first) - bin_edges.begin() - 1;
    ...
    bin_edges = std::move(bin_edges_new);
    bin_count = std::move(bin_count_new);
    bin_pos_count = std::move(bin_pos_count_new);
    bin_neg_count = std::move(bin_neg_count_new);
  }
  ...
  void apply_bin_cutoff() {
    int n_bins = bin_edges.size() - 1;
    double total_count = std::accumulate(bin_count.begin(), bin_count.end(), 0.0);
    for (int i = 0; i < n_bins; ++i) {
      if (bin_count[i] / total_count < bin_cutoff) {
        if (i > 0) {
          merge_bins(i - 1);
          --i;
          --n_bins;
...
#pragma omp parallel for
    for (size_t i = 0; i < feature.size(); ++i) {
      ...
      if (std::isfinite(val)) {
        ...
        woefeature[i] = 0.0;  // Handle non-finite values
      }
    }
  }
  
public:
  OptimalBinningNumericalBS(const std::vector<double>& feature_, const std::vector<int>& target_,
