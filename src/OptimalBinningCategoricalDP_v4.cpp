...
struct CategoryInfo {
  ...
  // Default constructor
  CategoryInfo() : count(0), pos_count(0), neg_count(0), event_rate(0.0) {}
 ...
 */
struct BinInfo {
  std::string name;      // Name of the bin (concatenated categories)
  double woe;            // Weight of Evidence
  double iv;             // Information Value
  int count;             // Total count
  int count_pos;         // Positive events count
  int count_neg;         // Negative events count
  double event_rate;     // Event rate (pos_count/count)
   ...
   */
  BinInfo(const std::string& name, double woe, double iv, int count, int count_pos, int count_neg)
    : name(name), woe(woe), iv(iv), count(count), count_pos(count_pos), count_neg(count_neg) {
...
private:
  ...
  const std::unordered_map<std::string, CategoryInfo>& category_info;
...
public:
   ...
   */
  BinCountCache(const std::string& separator, const std::unordered_map<std::string, CategoryInfo>& info)
  ...
  std::pair<int, int> get_bin_counts(const std::string& bin) {
    ...
    while ((end = bin.find(bin_separator, start)) != std::string::npos) {
      ...
      if (cat_it != category_info.end()) {
        ...
        counts.second += cat_it->second.pos_count;
      }
...
/**
 * @class OptimalBinningCategoricalDP
...
public:
  /**
   * @brief Constructor for OptimalBinningCategoricalDP
   * 
   ...
   */
  OptimalBinningCategoricalDP(const std::vector<std::string> &feature,
...
private:
  ...
  // Auxiliary variables
  std::unordered_map<std::string, CategoryInfo> category_info;
  double total_count;
  double total_pos;
  double total_neg;
  ...
  // Final results
  std::vector<BinInfo> bin_results;
  ...
  void compute_bins_no_optimization() {
    for (const auto& pair : category_info) {
      ...
      const CategoryInfo& info = pair.second;
      
      double woe, iv;
      compute_woe_iv(info.pos_count, info.neg_count, total_pos, total_neg, woe, iv);
      
      bin_results.emplace_back(cat, woe, iv, info.count, info.pos_count, info.neg_count);
    }
  ...
  void merge_rare_categories() {
    ...
    std::vector<std::pair<std::string, const CategoryInfo*>> sorted_cats;
    ...
    for (const auto& [cat, info] : sorted_cats) {
      if (info->count < cutoff_count) {
        ...
        current_pos_count += info->pos_count;
...
Rcpp::List optimal_binning_categorical_dp(
 ...
 // Execute algorithm
 OptimalBinningCategoricalDP binning(
