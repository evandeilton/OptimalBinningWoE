...
private:
  ...
  static double calculateWOE(int pos, int neg, int total_pos, int total_neg) {
    double adjusted_pos = std::max(static_cast<double>(pos), EPSILON);
    double adjusted_neg = std::max(static_cast<double>(neg), EPSILON);
    double pos_rate = adjusted_pos / total_pos;
    double neg_rate = adjusted_neg / total_neg;
    return std::log(pos_rate / neg_rate);
  }
  
  static double calculateIV(const std::vector<BinInfo>& bins, int total_pos, int total_neg) {
    double iv = 0.0;
    for (const auto& bin : bins) {
      double adjusted_pos = std::max(static_cast<double>(bin.count_pos), EPSILON);
      double adjusted_neg = std::max(static_cast<double>(bin.count_neg), EPSILON);
      double pos_rate = adjusted_pos / total_pos;
      double neg_rate = adjusted_neg / total_neg;
...
public:
  ...
  Rcpp::List fit() {
    ...
    for (const auto& bin : bins) {
      bin_names.push_back(joinStrings(bin.categories, "+"));
    ...
    for (const auto& bin : bins) {
      for (const auto& category : bin.categories) {
        category_woe_map[category] = bin.woe;
      }
    }
