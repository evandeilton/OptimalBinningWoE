...
void OptimalBinningCategoricalMOB::limitBins() {
  ...
  // If bins.size() < min_bins, need to merge further
  while (bins.size() < static_cast<size_t>(min_bins) && bins.size() > 1) {
    // Merge bins with minimum difference in WoE
    size_t min_index = 0;
    double min_diff = std::abs(bins[1].woe - bins[0].woe);
...
List OptimalBinningCategoricalMOB::fit() {
  ...
  for (size_t i = 0; i < feature.size(); ++i) {
    ...
    woefeature[i] = (it != category_to_woe.end()) ? it->second : NA_REAL;
  }
...
Rcpp::List optimal_binning_categorical_mob(Rcpp::IntegerVector target,
  ...
  try {
    OptimalBinningCategoricalMOB mob(std::move(feature_vec), std::move(target_vec), min_bins, max_bins, bin_cutoff, max_n_prebins);
    return mob.fit();
