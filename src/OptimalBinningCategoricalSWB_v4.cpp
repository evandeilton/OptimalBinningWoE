...
//     bins = std::move(temp_bins);
//     
//     std::sort(bins.begin(), bins.end(), [this](const BinStats& a, const BinStats& b) {
//        return a.woe < b.woe;
//     });
//     
//     while (bins.size() > (size_t)max_n_prebins && bins.size() > (size_t)min_bins) {
...
//     
//     while (iterations_run < max_iterations) {
//        if (is_monotonic(bins) && bins.size() <= (size_t)max_bins && bins.size() >= (size_t)min_bins) {
//           converged = true;
//           break;
//        }
//        
...
//           merge_adjacent_bins();
//        } else if (bins.size() < (size_t)min_bins) {
//           // Antes de tentar split, checar se é possível
//           // Se há 1 ou 2 categorias unicas e min_bins > ncat, não otimizar, apenas retornar
//           // Mas já estamos nessa etapa, melhor apenas sair do loop.
//           // Sem splitting se não for possível manter coerência sem travar a máquina.
//           // Saímos do loop para não travar a máquina.
//           break;
//        } else {
//           improve_monotonicity();
//        }
...
//     }
//     
//     // Caso ao final tenhamos menos bins que min_bins e não seja possível splitar,
//     // apenas aceite a situação atual. Não force splits que travem a máquina.
//     // Caso tenhamos mais bins que max_bins, tente mesclar novamente
//     while (bins.size() > (size_t)max_bins) {
//        merge_adjacent_bins();
//     }
//     
...
//     // Não implementaremos splitting complexo, pois pode criar comportamento indesejado.
//     // Em vez disso, se bins.size() < min_bins e não podemos splitar sem travar,
//     // apenas não fazer nada.
//     // Isso evitará travar a máquina.
//  }
//  
//  void improve_monotonicity() {
...
//     // Ajusta min_bins e max_bins com base no número de categorias
...
//  
//  void fit() {
//     // Contar níveis únicos antes
//     std::unordered_set<std::string> unique_cats(feature.begin(), feature.end());
//     int ncat = (int)unique_cats.size();
//     
//     // Se 1 ou 2 níveis, não otimiza, apenas calcula
//     if (ncat <= 2) {
//        // Cria bins diretamente
//        std::unordered_map<std::string, BinStats> bin_map;
//        total_pos = 0;
//        total_neg = 0;
//        for (size_t i = 0; i < feature.size(); ++i) {
...
//        }
//        
//        // Calcular WoE/IV sem otimização
//        for (auto& bin : bins) {
...
//     
//     Rcpp::NumericVector ids(bin_categories.size());
//     for(int i = 0; i < bin_categories.size(); i++) {
//       ids[i] = i + 1;
//     }
//     
//     return Rcpp::List::create(
