...
#pragma omp parallel
{
  std::map<std::string, Bin> local_bin_map;
  int local_total_pos = 0;
  int local_total_neg = 0;
...
#pragma omp for nowait
  for (size_t i = 0; i < target.size(); ++i) {
    ...
    if (local_bin_map.find(cat) == local_bin_map.end()) {
      local_bin_map[cat] = Bin{{cat}, 0, 0, 0.0, 0.0};
    }
    
    if (tar == 1) {
      local_bin_map[cat].count_pos++;
      local_total_pos++;
    } else {
      local_bin_map[cat].count_neg++;
      local_total_neg++;
    }
  }
...
#pragma omp critical
{
  for (const auto& kv : local_bin_map) {
    const std::string& cat = kv.first;
    const Bin& local_bin = kv.second;
    
    if (bin_map.find(cat) == bin_map.end()) {
      bin_map[cat] = local_bin;
    } else {
      bin_map[cat].count_pos += local_bin.count_pos;
      bin_map[cat].count_neg += local_bin.count_neg;
    }
  }
  total_pos += local_total_pos;
  total_neg += local_total_neg;
}
}
...
void OptimalBinningCategoricalMILP::merge_bins() {
  while (bins.size() > static_cast<size_t>(max_bins) || (bins.size() > static_cast<size_t>(min_bins) && bins.size() > static_cast<size_t>(max_n_prebins))) {
...
#pragma omp parallel
{
  double local_min_delta_iv = std::numeric_limits<double>::max();
  size_t local_merge_idx1 = 0;
  size_t local_merge_idx2 = 0;
...
#pragma omp for nowait
  for (size_t i = 0; i < bins.size(); ++i) {
    for (size_t j = i + 1; j < bins.size(); ++j) {
      ...
      if (delta_iv < local_min_delta_iv) {
        local_min_delta_iv = delta_iv;
        local_merge_idx1 = i;
        local_merge_idx2 = j;
      }
    }
...
#pragma omp critical
{
  if (local_min_delta_iv < min_delta_iv) {
    min_delta_iv = local_min_delta_iv;
    merge_idx1 = local_merge_idx1;
    merge_idx2 = local_merge_idx2;
  }
}
}
