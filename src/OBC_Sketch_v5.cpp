...
// =====================================================
// FIXED: MergeCache with proper initialization
// =====================================================
class MergeCache {
private:
  std::vector<std::vector<double>> iv_loss_cache;
  std::vector<std::vector<double>> divergence_cache;
  bool enabled_ = false;  // In-class initialization to guarantee default value
  size_t cache_size_ = 0;
  
  // Helper to check if indices are valid
  bool valid_indices(size_t bin1, size_t bin2) const {
    if (!enabled_) return false;
    if (cache_size_ == 0) return false;
    if (bin1 >= cache_size_ || bin2 >= cache_size_) return false;
    if (bin1 >= iv_loss_cache.size()) return false;
    if (bin1 >= divergence_cache.size()) return false;
    if (bin2 >= iv_loss_cache[bin1].size()) return false;
    if (bin2 >= divergence_cache[bin1].size()) return false;
    return true;
  }
  
public:
  // Default constructor
  MergeCache() : iv_loss_cache(), divergence_cache(), enabled_(false), cache_size_(0) {}
  
  // Parameterized constructor
  explicit MergeCache(size_t max_size, bool use_cache = true)
    : iv_loss_cache(), divergence_cache(), 
      enabled_(use_cache && max_size > 0), 
      cache_size_(max_size) {
    if (enabled_ && max_size > 0) {
      try {
        iv_loss_cache.resize(max_size);
        divergence_cache.resize(max_size);
        for (size_t i = 0; i < max_size; ++i) {
          iv_loss_cache[i].resize(max_size, -1.0);
          divergence_cache[i].resize(max_size, -1.0);
        }
      } catch (...) {
        // If allocation fails, disable caching
        enabled_ = false;
        cache_size_ = 0;
        iv_loss_cache.clear();
        divergence_cache.clear();
      }
    }
  }
  ...
  // Allow move
  MergeCache(MergeCache&& other) noexcept
    : iv_loss_cache(std::move(other.iv_loss_cache)),
      divergence_cache(std::move(other.divergence_cache)),
  ...
  MergeCache& operator=(MergeCache&& other) noexcept {
    if (this != &other) {
      iv_loss_cache = std::move(other.iv_loss_cache);
      divergence_cache = std::move(other.divergence_cache);
      enabled_ = other.enabled_;
  ...
  double get_iv_loss(size_t bin1, size_t bin2) const {
    if (!valid_indices(bin1, bin2)) {
      return -1.0;
    }
    return iv_loss_cache[bin1][bin2];
  }
  
  void set_iv_loss(size_t bin1, size_t bin2, double value) {
    if (!valid_indices(bin1, bin2)) {
      return;
    }
  ...
  double get_divergence(size_t bin1, size_t bin2) const {
    if (!valid_indices(bin1, bin2)) {
      return -1.0;
    }
  ...
  void set_divergence(size_t bin1, size_t bin2, double value) {
    if (!valid_indices(bin1, bin2)) {
      return;
    }
  ...
  void invalidate_bin(size_t bin_idx) {
    if (!enabled_ || bin_idx >= cache_size_) {
      return;
    }
    
    // Invalidate row
    if (bin_idx < iv_loss_cache.size()) {
      for (size_t i = 0; i < iv_loss_cache[bin_idx].size(); ++i) {
        iv_loss_cache[bin_idx][i] = -1.0;
      }
    }
    if (bin_idx < divergence_cache.size()) {
      for (size_t i = 0; i < divergence_cache[bin_idx].size(); ++i) {
    ...
    for (size_t i = 0; i < divergence_cache.size(); ++i) {
      if (bin_idx < divergence_cache[i].size()) {
  ...
  void resize(size_t new_size) {
    ...
    try {
      ...
      for (size_t i = 0; i < new_size; ++i) {
        iv_loss_cache[i].resize(new_size, -1.0);
        divergence_cache[i].resize(new_size, -1.0);
      }
      cache_size_ = new_size;
    } catch (...) {
      // If resize fails, disable caching
      enabled_ = false;
      cache_size_ = 0;
      iv_loss_cache.clear();
      divergence_cache.clear();
...
private:
  ...
  // Helper to ensure merge_cache is initialized
  void ensure_merge_cache() {
    if (!merge_cache) {
      size_t cache_size = bins.size();
      bool use_cache = cache_size > 10;
      merge_cache = std::make_unique<MergeCache>(cache_size, use_cache);
    }
  }
  ...
  void prebinning() {
    ...
    // Initialize merge cache with proper size
    size_t cache_size = bins.size();
    bool use_cache = cache_size > 10;
    merge_cache = std::make_unique<MergeCache>(cache_size, use_cache);
  ...
  void optimize_bins() {
    ...
    // Ensure merge cache is initialized
    ensure_merge_cache();
  ...
  void check_consistency() const {
    ...
    double pos_ratio = total_bad > 0 ? static_cast<double>(total_count_pos) / static_cast<double>(total_bad) : 0.0;
    double neg_ratio = total_good > 0 ? static_cast<double>(total_count_neg) / static_cast<double>(total_good) : 0.0;
...
public:
  OBC_Sketch(const std::vector<std::string> &feature_,
    ...
    : feature(feature_), target(Rcpp::as<std::vector<int>>(target_)),
      ...
      total_bad(0), merge_cache(nullptr) {
...
//     "N/A"; // Special category for missing values
// 
// // Namespace with optimized utility functions
// namespace utils {
...
//     std::sort(counts.begin(), counts.end());
// 
//     // Return {lower_bound, upper_bound} using median and min as conservative
//     // estimates
//     return {counts[0], counts[depth / 2]};
//   }
// 
...
//                 double threshold_ratio) const {
// 
//     // Improved total count estimation: average across all rows for better
//     // accuracy
//     int64_t total_count = 0;
//     for (size_t i = 0; i < depth; ++i) {
//       int64_t row_sum = std::accumulate(table[i].begin(), table[i].end(), 0LL);
...
// 
// public:
//   // Default constructor - ensures enabled is always initialized
...
//     if (!enabled || bin1 >= divergence_cache.size() ||
//         bin2 >= divergence_cache[bin1].size()) {
//       return -1.0;
//     }
//     return divergence_cache[bin1][bin2];
//   }
...
//     double heavy_hitter_threshold =
//         bin_cutoff / 2.0; // More permissive threshold for initial detection
//     std::vector<std::string> heavy_hitters =
//         sketch->heavy_hitters(unique_categories, heavy_hitter_threshold);
// 
...
//   OBC_Sketch sketch_binner(feature_vec, target, min_bins, max_bins, bin_cutoff,
//                            max_n_prebins, bin_separator, convergence_threshold,
//                            max_iterations, sketch_width, sketch_depth);
// 
//   return sketch_binner.fit();
// }
