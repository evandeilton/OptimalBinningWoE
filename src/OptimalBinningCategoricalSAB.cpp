...
private:
  ...
  void initialize() {
    ...
    actual_bins = std::min(std::max(std::min(n_categories, min_bins), 2), max_bins);
  ...
  double calculate_iv(const std::vector<int>& solution) {
    ...
    double iv = 0.0;
    double total_positive_rate = static_cast<double>(total_positive) / total_count;
    
    for (int i = 0; i < actual_bins; ++i) {
      if (bin_counts[i] > 0) {
        double bin_positive_rate = static_cast<double>(bin_positives[i]) / bin_counts[i];
        double bin_negative_rate = 1.0 - bin_positive_rate;
        double overall_negative_rate = 1.0 - total_positive_rate;
        
        if (bin_positive_rate > EPSILON && bin_positive_rate < 1.0 - EPSILON) {
          double woe = std::log1p((bin_positive_rate / total_positive_rate) - 1.0) -
            std::log1p((bin_negative_rate / overall_negative_rate) - 1.0);
          iv += (bin_positive_rate - bin_negative_rate) * woe;
        }
      }
    }
  ...
  static bool is_empty_bin(double rate) {
    return rate < EPSILON;
  }
  ...
  bool is_monotonic(const std::vector<int>& solution) {
    ...
    // Remove empty bins
    auto it = std::remove_if(bin_rates.begin(), bin_rates.end(), is_empty_bin);
    bin_rates.erase(it, bin_rates.end());
  ...
  std::vector<int> generate_neighbor(const std::vector<int>& solution) {
    ...
    int idx1 = std::uniform_int_distribution<>(0, neighbor.size() - 1)(gen);
    int idx2 = std::uniform_int_distribution<>(0, neighbor.size() - 1)(gen);
...
best_iv = calculate_iv(best_solution);  // Recalculate best_iv to ensure consistency
  ...
  Rcpp::List get_results() {
    ...
    double total_positive_rate = static_cast<double>(total_positive) / total_count;
    double total_negative_rate = 1.0 - total_positive_rate;
    ...
    for (int i = 0; i < actual_bins; ++i) {
      if (bin_counts[i] > 0) {
        ...
        double woe_value = std::log1p((bin_positive_rate / total_positive_rate) - 1.0) -
          std::log1p((bin_negative_rate / total_negative_rate) - 1.0);
        woe.push_back(woe_value);
        
        double iv_value = (bin_positive_rate - bin_negative_rate) * woe_value;
