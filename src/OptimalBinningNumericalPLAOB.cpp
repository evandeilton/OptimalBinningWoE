...
private:
  ...
  double calculate_woe(int count_pos, int count_neg, int total_pos, int total_neg) {
    double pos_rate = (count_pos + epsilon) / (total_pos + 2 * epsilon);
    double neg_rate = (count_neg + epsilon) / (total_neg + 2 * epsilon);
    return std::log(pos_rate / neg_rate);
  }
  
  double calculate_iv(double woe, double pos_rate, double neg_rate) {
    return (pos_rate - neg_rate) * woe;
  }
  
  void create_initial_bins() {
    // Sort the feature values along with their target values
    std::vector<std::pair<double, int>> data(feature.size());
    ...
    int n = data.size();
    int initial_bins = std::min(max_n_prebins, n);
    
    int bin_size = n / initial_bins;
    int remainder = n % initial_bins;
    int index = 0;
    ...
    for (int i = 0; i < initial_bins; ++i) {
      ...
      Bin bin;
      bin.lower = (i == 0) ? -std::numeric_limits<double>::infinity() : data[index].first;
      bin.upper = (i == initial_bins - 1) ? std::numeric_limits<double>::infinity() : data[index + current_bin_size - 1].first;
      bin.count = current_bin_size;
      bin.count_pos = 0;
      bin.count_neg = 0;
      
      for (int j = 0; j < current_bin_size; ++j) {
        int tgt = data[index + j].second;
        if (tgt == 1) bin.count_pos++;
        else bin.count_neg++;
      }
      
      bins.push_back(bin);
      index += current_bin_size;
    }
  }
  ...
  void merge_low_frequency_bins() {
    ...
    size_t i = 0;
    while (i < bins.size()) {
      if (bins[i].count < cutoff_count && bins.size() > static_cast<size_t>(min_bins)) {
        if (i == 0) {
          // Merge with next bin
          merge_bins(i, i + 1);
        } else {
  ...
  void merge_bins(size_t index1, size_t index2) {
    bins[index1].upper = bins[index2].upper;
  ...
  void calculate_woe_iv() {
    ...
    for (auto& bin : bins) {
      double pos_rate = (bin.count_pos + epsilon) / (total_pos + 2 * epsilon);
      double neg_rate = (bin.count_neg + epsilon) / (total_neg + 2 * epsilon);
  ...
  void enforce_monotonicity() {
    ...
    size_t i = 1;
    while (i < bins.size()) {
      bool condition = is_increasing ? bins[i].woe >= bins[i - 1].woe : bins[i].woe <= bins[i - 1].woe;
      if (!condition && bins.size() > static_cast<size_t>(min_bins)) {
        merge_bins(i - 1, i);
        i = (i > 1) ? i - 1 : 1;
      } else {
        i++;
  ...
  void split_bin() {
    ...
    auto max_bin_it = std::max_element(bins.begin(), bins.end(),
                                       [](const Bin& a, const Bin& b) { return a.count < b.count; });
    size_t index = std::distance(bins.begin(), max_bin_it);
    
    if (bins[index].lower == bins[index].upper || !std::isfinite(bins[index].lower) || !std::isfinite(bins[index].upper)) {
      // Cannot split this bin
      return;
    }
    
    double mid = (bins[index].lower + bins[index].upper) / 2.0;
    Bin left_bin = bins[index];
    Bin right_bin = bins[index];
    left_bin.upper = mid;
    right_bin.lower = mid;
    ...
    bins.insert(bins.begin() + index, right_bin);
    bins.insert(bins.begin() + index, left_bin);
  }
  
  void merge_smallest_bins() {
    if (bins.size() <= 1) return;
    
    // Find adjacent bins with smallest total count
    size_t merge_index = 0;
    int min_total_count = bins[0].count + bins[1].count;
    for (size_t i = 1; i < bins.size() - 1; ++i) {
      int total_count = bins[i].count + bins[i + 1].count;
...
public:
  ...
  Rcpp::List fit() {
    ...
    if (total_pos == 0 || total_neg == 0) {
      Rcpp::Rcout << "Warning: Target variable has only one class. WoE and IV calculations may not be meaningful.\n";
      return Rcpp::List::create(
        Rcpp::Named("woefeature") = Rcpp::NumericVector(feature.size(), 0.0),
...
#pragma omp parallel for
    ...
    for (size_t i = 0; i < feature.size(); ++i) {
      for (const auto& bin : bins) {
        if (feature[i] > bin.lower && feature[i] <= bin.upper) {
