...
/**
 * WoE from smoothed class distributions, capped at +/-20 when one of them is
 * zero (possible only with laplace_smoothing = 0 and a pure bin).
 */
static inline double mdlp_capped_woe(double dist_pos, double dist_neg) {
  if (dist_pos <= 0.0) return (dist_neg <= 0.0) ? 0.0 : -20.0;
  if (dist_neg <= 0.0) return 20.0;
...
private:
  ...
  // MDL cost. Only its change under a merge is ever needed, and that is
  // computed analytically in apply_mdl_merging()/enforce_max_bins():
  //   MDL(k) = log2(k - 1) + sum_i n_i * H(S_i)
  // (model cost plus the bits needed to encode the classes given the bins).
  // The former full-cost helper was dead code -- its only call site discarded
  // the result -- and it carried the opposite sign on the data term.
  ...
  void merge_bins(size_t index) {
    if (index + 1 >= bins.size()) {
      Rcpp::stop("Internal error: invalid bin index for merging.");
    }
  ...
  void calculate_woe_iv() {
    ...
    // Calculate WoE and IV for each bin (the total is summed in get_results())
    for (auto &bin : bins) {
      ...
      double total_smoothed_pos = total_pos + static_cast<double>(bins.size()) * laplace_smoothing;
      double total_smoothed_neg = total_neg + static_cast<double>(bins.size()) * laplace_smoothing;

      double dist_pos = smoothed_pos / total_smoothed_pos;
      double dist_neg = smoothed_neg / total_smoothed_neg;

      // Calculate WoE with protection against edge cases (capped, so the
      // WoE and the IV contribution are always finite)
      bin.woe = mdlp_capped_woe(dist_pos, dist_neg);
  ...
  void enforce_monotonicity() {
    ...
    // Class totals do not change under merging, so they are computed once.
    int total_pos = 0, total_neg = 0;
    for (const auto& bin : bins) {
      total_pos += bin.count_pos;
      total_neg += bin.count_neg;
    ...
    while (!monotonic && iterations_run < max_iterations) {
      ...
      for (size_t i = 1; i < bins.size(); ++i) {
        ...
        if (violation) {
          ...
          // IV of the bin that merging (i-1, i) would produce, evaluated with
          // the post-merge bin count. Only that one bin is needed, so no copy
          // of the whole bin vector is made per violation any more.
          const int m_pos = bins[i-1].count_pos + bins[i].count_pos;
          const int m_neg = bins[i-1].count_neg + bins[i].count_neg;
          const double k_after = static_cast<double>(bins.size() - 1);
          double smoothed_pos = m_pos + laplace_smoothing;
          double smoothed_neg = m_neg + laplace_smoothing;
          double total_smoothed_pos = total_pos + k_after * laplace_smoothing;
          double total_smoothed_neg = total_neg + k_after * laplace_smoothing;
          double dist_pos = smoothed_pos / total_smoothed_pos;
          double dist_neg = smoothed_neg / total_smoothed_neg;

          double new_woe = mdlp_capped_woe(dist_pos, dist_neg);
          double new_iv = (dist_pos - dist_neg) * new_woe;
          ...
          // merge_bins() pools the counts only. Without this refresh the
          // merged bin kept the WoE of its left half (and every other bin the
          // smoothing of the previous bin count), so the next pass tested
          // stale values and could stop on a sequence that was not monotone.
          calculate_woe_iv();
    ...
    if (!monotonic && iterations_run >= max_iterations) {
      converged = false;
  ...
  void merge_rare_bins() {
    ...
    while (merged && iterations_run < max_iterations && bins.size() > (size_t)min_bins) {
      ...
      if (min_freq < bin_cutoff && min_idx < bins.size()) {
        ...
        if (min_idx == 0) {
          ...
          // Use entropy to decide merge direction. Only the pooled class
          // counts of each candidate are needed (the two full copies of the
          // bin vector made here per merge were never read beyond them).
          const NumericalBin& lo = bins[min_idx - 1];
          const NumericalBin& mid = bins[min_idx];
          const NumericalBin& hi = bins[min_idx + 1];

          // Calculate entropy for merged bins
          double entropy_left_merged = calculate_entropy(lo.count_pos + mid.count_pos,
                                                         lo.count_neg + mid.count_neg);
          double entropy_right_merged = calculate_entropy(mid.count_pos + hi.count_pos,
                                                          mid.count_neg + hi.count_neg);
    ...
    // Only report non-convergence when the cap actually left work undone: a
    // rare bin that could still be merged. The old test fired whenever the
    // shared iteration counter was already exhausted on entry, even with no
    // rare bin at all.
    if (iterations_run >= max_iterations && bins.size() > (size_t)min_bins) {
      bool rare_left = false;
      for (const auto& b : bins) {
        if (static_cast<double>(b.count) / total < bin_cutoff) {
          rare_left = true;
          break;
        }
      }
...
public:
  ...
  laplace_smoothing(laplace_smoothing_), iterations_run(0), converged(true) {

    // Input validation
    validate_inputs();

    // The constructor used to build a std::set of every feature value here to
    // "adjust min_bins/max_bins to the number of unique values". The
    // parameters shadowed the members, so the adjustment only ever touched
    // the local copies and had no effect, while the set cost O(n log n) with
    // a node allocation per value (most of the run time at n = 10^6) and
    // ordered NaN with operator<, which is undefined behaviour. Neither
    // adjustment is needed: pre-binning never yields more bins than distinct
    // values, and the merge loops already stop at min_bins.
  }
  ...
  void fit() {
    ...
    // Every feature value was NaN: there is nothing to bin. (This used to
    // fall through to calculate_woe_iv() on an empty bin vector and fail
    // with the misleading "All target values are the same".)
    if (sorted_data.empty()) {
      Rcpp::stop("All feature values are NA/NaN; nothing to bin.");
    }
    ...
    // (The former validate_bins() pass re-checked ordering, contiguity and
    // the +/-Inf ends, all guaranteed by construction; the output contract is
  ...
  bool handle_special_cases() {
    // (An empty feature vector is rejected by validate_inputs().)
    // All the same target value
  ...
  void handle_few_unique_values(const std::vector<std::pair<double, int>>& sorted_data, int actual_unique) {
    ...
    // Two unique values - create two bins
      ...
      // Create two bins. The cut is the lower value itself (right-closed
      // bins), except when that value is -Inf: a -Inf cutpoint is not a
      // usable boundary, and the most negative finite double separates the
      // two values identically.
      const double cut = (uniq_vals[0] == -std::numeric_limits<double>::infinity())
        ? std::numeric_limits<double>::lowest()
        : uniq_vals[0];
      NumericalBin bin1, bin2;
      bin1.lower_bound = -std::numeric_limits<double>::infinity();
      bin1.upper_bound = cut;
      bin1.count = 0; bin1.count_pos = 0; bin1.count_neg = 0;

      bin2.lower_bound = cut;
  ...
  void create_prebins(const std::vector<std::pair<double, int>>& sorted_data) {
    ...
    for (size_t i = 0; i < sorted_data.size(); ) {
      ...
      //
      // A pre-bin may not close on -Inf either: its upper bound would become
      // a cutpoint of -Inf, labelled "(-Inf;-inf]". Such a bin absorbs the
      // following run of values instead.
      while (end < sorted_data.size() &&
             (sorted_data[end].first == sorted_data[end - 1].first ||
              sorted_data[end - 1].first == -std::numeric_limits<double>::infinity())) {
        ++end;
      }
  ...
  void apply_mdl_merging() {
    // True once the loop stops for an algorithmic reason (MDL optimum, bin
    // budget met, or min_bins reached) rather than on the iteration cap.
    bool finished = false;
    ...
    // Warn only when the cap really cut the phase short. The old test fired
    // whenever the counter had reached the cap, including after the loop had
    // already stopped at min_bins or at the MDL optimum.
    if (!finished && bins.size() > (size_t)min_bins &&
