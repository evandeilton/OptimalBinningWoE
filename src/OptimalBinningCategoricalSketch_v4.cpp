...
// }
// 
// // Função de hash universal para strings
// inline size_t string_hash(const std::string& str, size_t seed) {
//   size_t hash = seed;
//   for (char c : str) {
//     hash = hash * 31 + c;
...
// }
// 
// // Estrutura de Count-Min Sketch para estimativa de frequências
// class CountMinSketch {
// private:
//   std::vector<std::vector<int>> table;
//   std::vector<size_t> seeds;
...
//     }
//     
//     // Inicializar seeds para funções hash
//     std::mt19937 gen(42); // Semente fixa para reprodutibilidade
//     std::uniform_int_distribution<size_t> dist(1, std::numeric_limits<size_t>::max());
//     
//     seeds.resize(depth);
//     for (size_t i = 0; i < depth; ++i) {
...
//   }
//   
//   // Incrementar contador para um item
//   void update(const std::string& item, int count = 1) {
//     for (size_t i = 0; i < depth; ++i) {
//       size_t hash = utils::string_hash(item, seeds[i]) % width;
//       table[i][hash] += count;
...
//   }
//   
//   // Método para detectar categorias frequentes (heavy hitters)
//   std::vector<std::string> heavy_hitters(
//       const std::vector<std::string>& candidates, 
//       double threshold_ratio) const {
//     
//     // Estimar total de elementos no sketch
//     int total_count = 0;
//     for (size_t i = 0; i < width; ++i) {
//       total_count += table[0][i];
//     }
//     
//     // Threshold absoluto baseado na relação
//     int threshold = static_cast<int>(total_count * threshold_ratio);
//     
//     // Filtrar candidatos pelo threshold
//     std::vector<std::string> result;
//     result.reserve(candidates.size() / 4);
//     
//     for (const auto& candidate : candidates) {
...
//   }
//   
//   // Método para adicionar categoria com contagem estimada do sketch
//   inline void add_category_with_counts(const std::string& cat, int pos_count, int neg_count) {
//     // Adiciona apenas se é uma categoria nova
//     if (category_set.insert(cat).second) {
//       categories.push_back(cat);
...
// };
// 
// // Classe principal para Categorical Sketch
// class OptimalBinningCategoricalSketch {
// private:
//   std::vector<std::string> feature;
//   std::vector<int> target;
...
//   std::unique_ptr<MergeCache> merge_cache;
//   std::unique_ptr<CountMinSketch> sketch;
//   std::unique_ptr<CountMinSketch> sketch_pos;  // Para eventos positivos
//   std::unique_ptr<CountMinSketch> sketch_neg;  // Para eventos negativos
//   
...
//   }
//   
//   // Preencher sketches com os dados
//   void build_sketches() {
...
//     total_bad = 0;
//     
//     // Conjunto para rastrear categorias únicas
//     std::unordered_set<std::string> unique_categories;
//     
//     // Preencher sketches em uma única passagem
//     for (size_t i = 0; i < feature.size(); ++i) {
//       const auto& cat = feature[i];
//       int is_positive = target[i];
//       
//       unique_categories.insert(cat);
//       
//       // Atualizar sketch geral
//       sketch->update(cat, 1);
//       
//       // Atualizar sketch para positivos/negativos
//       if (is_positive) {
//         sketch_pos->update(cat, 1);
//         total_bad++;
//       } else {
...
//     }
//     
//     // Converter para vetor para facilitar processamento
//     std::vector<std::string> unique_categories(unique_categories_set.begin(), unique_categories_set.end());
//     
//     // Identificar heavy hitters utilizando Count-Min Sketch
//     double heavy_hitter_threshold = bin_cutoff / 2.0;  // Threshold mais permissivo para detecção inicial
//     std::vector<std::string> heavy_hitters = sketch->heavy_hitters(unique_categories, heavy_hitter_threshold);
//     
//     // Criar bins iniciais a partir dos heavy hitters
//     bins.clear();
//     bins.reserve(heavy_hitters.size());
//     
//     for (const auto& cat : heavy_hitters) {
...
//     }
//     
//     // Adicionar categorias raras a um bin separado
//     Bin rare_bin;
//     for (const auto& cat : unique_categories) {
//       if (std::find(heavy_hitters.begin(), heavy_hitters.end(), cat) == heavy_hitters.end()) {
//         int pos_count = sketch_pos->estimate(cat);
...
//     }
//     
//     // Adicionar bin de categorias raras se não estiver vazio
//     if (rare_bin.count > 0) {
//       bins.push_back(rare_bin);
//     }
//     
//     // Ordenar bins por contagem (decrescente)
//     std::sort(bins.begin(), bins.end(),
//               [](const Bin& a, const Bin& b) { return a.count > b.count; });
//     
...
//     }
//     
//     // Validação mais permissiva devido à aproximação do sketch
//     double count_ratio = static_cast<double>(total_count) / feature.size();
//     if (count_ratio < 0.95 || count_ratio > 1.05) {
//       Rcpp::warning(
//         "Possível inconsistência após binagem devido à aproximação do sketch. "
...
//     if (pos_ratio < 0.95 || pos_ratio > 1.05 || neg_ratio < 0.95 || neg_ratio > 1.05) {
//       Rcpp::warning(
//         "Possível inconsistência em contagens positivas/negativas após binagem devido à aproximação do sketch. "
...
//       );
//     } catch (const std::exception& e) {
//       Rcpp::stop("Erro no binning ótimo com sketch: " + std::string(e.what()));
//     }
//   }
// };
// 
...
// //' \enumerate{
// //'   \item Input validation and preprocessing
// //'   \item Building frequency sketches for the data
// //'   \item Initial pre-binning based on frequency estimates
// //'   \item Enforcing minimum bin size (bin_cutoff)
// //'   \item Calculating initial Weight of Evidence (WOE) and Information Value (IV)
// //'   \item Enforcing monotonicity of WOE across bins
