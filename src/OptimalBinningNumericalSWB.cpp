...
private:
  ...
  int count_unique(const NumericVector& vec);
...
int OptimalBinningNumericalSWB::count_unique(const NumericVector& vec) {
  std::set<double> unique_vals(vec.begin(), vec.end());
  return unique_vals.size();
}

OptimalBinningNumericalSWB::OptimalBinningNumericalSWB(NumericVector feature, IntegerVector target, int min_bins_input, int max_bins_input, double bin_cutoff_input, int max_n_prebins_input)
  : feature(feature), target(target), min_bins(min_bins_input), max_bins(max_bins_input), bin_cutoff(bin_cutoff_input), max_n_prebins(max_n_prebins_input) {
  ...
  if (is_true(any(is_na(feature)))) {
    stop("Feature contains NA values.");
  }
  for (int i = 0; i < feature.size(); ++i) {
    if (std::isinf(feature[i])) {
      stop("Feature contains Inf values.");
    }
  }
  
  // Check that target contains only 0 and 1
  IntegerVector unique_target = sort_unique(target);
  if (unique_target.size() != 2 || unique_target[0] != 0 || unique_target[1] != 1) {
    stop("Target variable must be binary (0 or 1).");
  }
  
  // Check for constant feature
  if (is_true(all(feature == feature[0]))) {
    stop("Feature has constant value; binning is not possible.");
  }
  
  // Check for single-class target (already ensured target has both 0 and 1)
  
  // Handle features with two or fewer unique categories
  int unique_categories = count_unique(feature);
  if (unique_categories <= 2) {
    // Set min_bins to the number of unique categories
    min_bins = unique_categories;
    if (max_bins < min_bins) {
      max_bins = min_bins;
      Rcpp::warning("max_bins adjusted to match the number of unique categories (%d).", max_bins);
    }
  }
}
...
void OptimalBinningNumericalSWB::calculate_woe_iv() {
  ...
  for (size_t i = 0; i < bin_counts.size(); ++i) {
    ...
    // Ensure distributions are not zero to avoid division by zero
    ...
    // Create bin labels using semicolon for standard interval notation
    std::ostringstream oss_left, oss_right;
    oss_left.precision(6);
    oss_left << std::fixed;
    oss_right.precision(6);
    oss_right << std::fixed;
    
    if (bin_edges[i] == -std::numeric_limits<double>::infinity()) {
      oss_left << "(-Inf";
    } else {
      oss_left << "(" << bin_edges[i];
    }
    
    if (bin_edges[i + 1] == std::numeric_limits<double>::infinity()) {
      oss_right << "+Inf]";
    } else {
      oss_right << bin_edges[i + 1] << "]";
    }
    
    bin_labels[i] = oss_left.str() + ";" + oss_right.str();
  }
}
...
void OptimalBinningNumericalSWB::enforce_monotonicity(int max_iterations) {
  ...
  while (iteration < max_iterations) {
    ...
    // If the number of bins is less than or equal to min_bins, allow deviation from monotonicity
    if (bin_counts.size() <= static_cast<size_t>(min_bins)) {
      Rcpp::warning("Monotonicity could not be enforced without violating min_bins constraint.");
      break;
    }
...
void OptimalBinningNumericalSWB::fit() {
  ...
  int unique_categories = count_unique(feature);
  if (unique_categories <= 2) {
    // Directly assign WoE based on unique categories
    ...
    // Identify unique values and initialize counts
    ...
    for (auto val : unique_set) {
      ...
      pos_counts.push_back(0);
      neg_counts.push_back(0);
    }
    
    // Count positives and negatives for each unique value
    for (int i = 0; i < n; ++i) {
      double val = feature[i];
      int idx = value_to_index[val];
      if (target[i] == 1) {
        pos_counts[idx]++;
      } else {
        neg_counts[idx]++;
    ...
    double total_pos = std::accumulate(pos_counts.begin(), pos_counts.end(), 0.0);
    double total_neg = std::accumulate(neg_counts.begin(), neg_counts.end(), 0.0);
    
    // Calculate WoE
    std::vector<double> woe(unique_categories, 0.0);
    for (int i = 0; i < unique_categories; ++i) {
      double dist_pos = pos_counts[i] / total_pos;
      double dist_neg = neg_counts[i] / total_neg;
      if (dist_pos == 0) dist_pos = EPSILON;
      if (dist_neg == 0) dist_neg = EPSILON;
    ...
    for (int i = 0; i < n; ++i) {
      double val = feature[i];
      int idx = value_to_index[val];
      woefeature[i] = woe[idx];
    }
    
    // Prepare woebin DataFrame
    std::vector<std::string> bin_labels_local;
    std::vector<double> bin_woe_local;
    std::vector<double> bin_iv_local;
    std::vector<int> bin_counts_local;
    std::vector<int> bin_pos_counts_local;
    std::vector<int> bin_neg_counts_local;
    
    for (int i = 0; i < unique_categories; ++i) {
      // Create bin labels using the unique values
      std::ostringstream oss_label;
      oss_label << unique_values[i];
      bin_labels_local.push_back(oss_label.str());
      bin_woe_local.push_back(woe[i]);
      // Information Value (IV) calculation
      double dist_pos = pos_counts[i] / total_pos;
      double dist_neg = neg_counts[i] / total_neg;
      bin_iv_local.push_back((dist_pos - dist_neg) * woe[i]);
      bin_counts_local.push_back(pos_counts[i] + neg_counts[i]);
      bin_pos_counts_local.push_back(pos_counts[i]);
      bin_neg_counts_local.push_back(neg_counts[i]);
    }
    
    woebin = DataFrame::create(
      Named("bin") = bin_labels_local,
      Named("woe") = bin_woe_local,
      Named("iv") = bin_iv_local,
      Named("count") = bin_counts_local,
      Named("count_pos") = bin_pos_counts_local,
      Named("count_neg") = bin_neg_counts_local
    );
    
    return; // Binning bypassed for binary features
  }
  ...
  bin_pos_counts.assign(num_bins, 0);
  bin_neg_counts.assign(num_bins, 0);
...
#pragma omp parallel for
  ...
  while (i_bin < num_bins) {
    ...
    double current_left_edge = bin_edges[i_bin];
...
//' @export
// [[Rcpp::export]]
List optimal_binning_numerical_swb(IntegerVector target, 
                                   NumericVector feature, int min_bins = 3,
