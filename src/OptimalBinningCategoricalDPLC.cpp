...
private:
  ...
  int get_bin_count(const std::string &bin) const {
    int count = 0;
    std::vector<std::string> categories = split_string(bin, DELIMITER);
    ...
    for (const auto &cat : sorted_categories_count) {
      if (cat.second < cutoff_count) {
        if (current_merged.empty()) {
          ...
          current_merged += DELIMITER + cat.first;
        }
        current_count += cat.second;
      } else {
        if (!current_merged.empty()) {
          merged_categories.push_back(current_merged);
          std::vector<std::string> split_cats = split_string(current_merged, DELIMITER);
  ...
  void ensure_max_prebins() {
    while (merged_categories.size() > static_cast<size_t>(max_n_prebins)) {
      ...
      std::string merged_bin = smallest_bin + DELIMITER + *min_it2;
      *min_it2 = merged_bin;
      
      std::vector<std::string> split_cats = split_string(merged_bin, DELIMITER);
  ...
  void compute_and_sort_event_rates() {
    ...
    for (const auto &bin : merged_categories) {
      double pos = 0.0;
      double total = 0.0;
      std::vector<std::string> split_cats = split_string(bin, DELIMITER);
      for (const auto &cat : split_cats) {
  ...
  void initialize_dp_structures() {
    ...
    for (size_t i = 0; i < n; ++i) {
      double pos = 0.0;
      double neg = 0.0;
      std::vector<std::string> split_cats = split_string(sorted_categories[i], DELIMITER);
      for (const auto &cat : split_cats) {
        pos += category_pos_counts[cat];
