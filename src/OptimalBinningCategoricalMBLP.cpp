...
private:
  ...
  void preprocess_data(const std::vector<std::string>& feature, const std::vector<int>& target) {
    std::vector<std::map<std::string, std::pair<int, int>>> thread_local_stats;
    
#ifdef _OPENMP
    int num_threads = omp_get_max_threads();
    thread_local_stats.resize(num_threads);
#else
    thread_local_stats.resize(1);
#endif
...
#pragma omp parallel for
    for (size_t i = 0; i < feature.size(); ++i) {
      ...
      if (target[i] == 1) {
        thread_local_stats[thread_id][feature[i]].first++;
      } else if (target[i] == 0) {
        thread_local_stats[thread_id][feature[i]].second++;
      } else {
    ...
    for (const auto& thread_stats : thread_local_stats) {
      for (const auto& stat : thread_stats) {
        category_stats[stat.first].first += stat.second.first;
        category_stats[stat.first].second += stat.second.second;
      }
    }
    ...
    if (total_positive == 0 || total_negative == 0) {
      throw std::invalid_argument("Target must have both positive and negative samples");
    }
  }
  ...
  void merge_rare_categories() {
    ...
    // Ensure we don't have more categories than max_n_prebins
    if (unique_categories.size() > static_cast<size_t>(max_n_prebins)) {
      throw std::runtime_error("Number of unique categories exceeds max_n_prebins after merging rare categories");
    }
  }
  ...
  void optimize_bins() {
    ...
    while (bins.size() > static_cast<size_t>(max_bins) || (bins.size() > static_cast<size_t>(min_bins) && can_merge_bins(bins))) {
  ...
  double calculate_woe(int positive, int negative) {
    ...
    if (pos_rate == 0) pos_rate = std::numeric_limits<double>::min();
    if (neg_rate == 0) neg_rate = std::numeric_limits<double>::min();
