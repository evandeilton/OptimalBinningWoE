...
private:
  ...
  // Utility methods
  bool is_monotonic_increasing(const std::vector<double>& vec);
  bool is_monotonic_decreasing(const std::vector<double>& vec);
};
...
void OptimalBinningNumericalGAB::prebin_data() {
  // Pre-bin data into max_n_prebins bins based on quantiles
  std::vector<double> sorted_feature = feature_;
  std::sort(sorted_feature.begin(), sorted_feature.end());
  
  std::vector<double> cut_points;
  for (int i = 1; i < max_n_prebins_; ++i) {
    size_t idx = static_cast<size_t>(i * sorted_feature.size() / max_n_prebins_);
  ...
  cut_points.erase(std::unique(cut_points.begin(), cut_points.end(),
                               [](double a, double b) { return std::abs(a - b) < 1e-9; }), cut_points.end());
  ...
  for (int i = 0; i < population_size_; ++i) {
    ...
    if (cut_points.size() > static_cast<size_t>(n_bins - 1)) {
      ...
      while (cut_points.size() < static_cast<size_t>(n_bins - 1)) {
        ...
        if (std::find_if(cut_points.begin(), cut_points.end(),
                         [&](double val) { return std::abs(val - cp) < 1e-9; }) == cut_points.end()) {
...
void OptimalBinningNumericalGAB::selection() {
  ...
  // Elitism: retain top 50% individuals
  size_t num_elites = static_cast<size_t>(population_size_ * 0.5);
  if (population_.size() > num_elites) {
...
void OptimalBinningNumericalGAB::crossover() {
  ...
  while (offspring.size() + population_.size() < static_cast<size_t>(population_size_)) {
    // Select two parents
    ...
    for (size_t cp_idx = 0; cp_idx < parent1.cut_points.size(); ++cp_idx) {
      double gene = (std::bernoulli_distribution(0.5)(rng_)) ? parent1.cut_points[cp_idx] : parent2.cut_points[cp_idx];
      child_cut_points.push_back(gene);
    }
    
    // If parents have different numbers of cut points
    if (parent1.cut_points.size() != parent2.cut_points.size()) {
      // Randomly add or remove cut points to match a valid number
      size_t target_size = std::min(parent1.cut_points.size(), parent2.cut_points.size());
      while (child_cut_points.size() > target_size) {
        child_cut_points.pop_back();
      }
      while (child_cut_points.size() < target_size) {
        std::uniform_real_distribution<double> cut_point_dist(*std::min_element(feature_.begin(), feature_.end()),
                                                              *std::max_element(feature_.begin(), feature_.end()));
        double cp = cut_point_dist(rng_);
        if (std::find_if(child_cut_points.begin(), child_cut_points.end(),
                         [&](double val) { return std::abs(val - cp) < 1e-9; }) == child_cut_points.end()) {
          child_cut_points.push_back(cp);
        }
      }
    }
    ...
    // Ensure the number of bins is within limits
  ...
  population_.insert(population_.end(), offspring.begin(), offspring.end());
}
...
void OptimalBinningNumericalGAB::mutation() {
  ...
  for (Individual& ind : population_) {
    if (mutation_dist(rng_) < mutation_rate_) {
      // Mutate a random cut point
      if (!ind.cut_points.empty()) {
        std::uniform_int_distribution<size_t> cp_idx_dist(0, ind.cut_points.size() - 1);
        size_t cp_idx = cp_idx_dist(rng_);
        double new_cp = cut_point_dist(rng_);
        ind.cut_points[cp_idx] = new_cp;
        std::sort(ind.cut_points.begin(), ind.cut_points.end());
        // Remove duplicates
        ind.cut_points.erase(std::unique(ind.cut_points.begin(), ind.cut_points.end(),
                                         [&](double a, double b) { return std::abs(a - b) < 1e-9; }), ind.cut_points.end());
...
bool OptimalBinningNumericalGAB::check_monotonicity(const std::vector<double>& cut_points) {
  ...
  std::vector<double> woe(n_bins, 0.0);
  for (size_t i = 0; i < n_bins; ++i) {
    double dist_pos = (count_pos[i] + 0.5) / (total_pos + 0.5 * n_bins);
    double dist_neg = (count_neg[i] + 0.5) / (total_neg + 0.5 * n_bins);
...
void OptimalBinningNumericalGAB::adjust_bins_for_cutoff(std::vector<double>& cut_points) {
  ...
  do {
    ...
    double total_count = static_cast<double>(feature_.size());
    for (size_t i = 0; i < n_bins; ++i) {
...
bool OptimalBinningNumericalGAB::is_monotonic_increasing(const std::vector<double>& vec) {
...
bool OptimalBinningNumericalGAB::is_monotonic_decreasing(const std::vector<double>& vec) {
  for (size_t i = 1; i < vec.size(); ++i) {
    if (vec[i] > vec[i - 1]) {
      return false;
    }
...
void OptimalBinningNumericalGAB::fit() {
  ...
  std::vector<double> woe_values(n_bins, 0.0);
  std::vector<double> iv_values(n_bins, 0.0);
  std::vector<double> counts(n_bins, 0.0);
  std::vector<double> count_pos_vec(n_bins, 0.0);
  std::vector<double> count_neg_vec(n_bins, 0.0);
  ...
  if (n_bins < static_cast<size_t>(min_bins_)) {
    Rcpp::stop("Number of bins after fitting is less than min_bins.");
  }
  if (n_bins > static_cast<size_t>(max_bins_)) {
    Rcpp::stop("Number of bins after fitting exceeds max_bins.");
  }
}
...
Rcpp::List optimal_binning_numerical_gab(const Rcpp::NumericVector& target,
 ...
 OptimalBinningNumericalGAB obng(target_vec, feature_vec, min_bins, max_bins, bin_cutoff, max_n_prebins, population_size, max_generations, mutation_rate);
 obng.fit();
