...
// Utility function to format bin labels
std::string formatBinLabel(double lower, double upper, bool is_first, bool is_last) {
...
private:
  ...
  std::vector<R_xlen_t> bin_counts;
  std::vector<R_xlen_t> bin_count_pos;
  std::vector<R_xlen_t> bin_count_neg;
  std::vector<std::string> bin_labels;
  
public:
  OptimalBinningNumericalMBA(NumericVector feature, IntegerVector target,
                             R_xlen_t min_bins, R_xlen_t max_bins, double bin_cutoff,
                             R_xlen_t max_n_prebins, int n_threads)
  ...
  void fit() {
    ...
    // Final check to ensure bin constraints
    enforceBinConstraints();
  }
...
#pragma omp parallel for num_threads(n_threads)
    for (R_xlen_t i = 0; i < feature.size(); ++i) {
      ...
      R_xlen_t bin_idx = findBin(val);
      // Handle edge cases where bin_idx might be out of range
      if (bin_idx < 0) bin_idx = 0;
      if (bin_idx >= static_cast<R_xlen_t>(woe_values.size())) bin_idx = woe_values.size() - 1;
...
private:
  void prebinning() {
    ...
    R_xlen_t n = sorted_feature.size();
    R_xlen_t n_bins = std::min(max_n_prebins, n);
    
    // Use quantiles to get bin edges
    NumericVector quantiles = NumericVector(static_cast<int>(n_bins - 1));
    for (R_xlen_t i = 1; i < n_bins; ++i) {
      double q = static_cast<double>(i) / n_bins;
      double pos = std::floor(q * (n - 1));
      quantiles[i - 1] = sorted_feature[static_cast<int>(pos)];
    }
    ...
    R_xlen_t n_bins_actual = bin_edges.size() - 1;
    bin_counts = std::vector<R_xlen_t>(n_bins_actual, 0);
    bin_count_pos = std::vector<R_xlen_t>(n_bins_actual, 0);
    bin_count_neg = std::vector<R_xlen_t>(n_bins_actual, 0);
...
#pragma omp parallel for num_threads(n_threads)
    for (R_xlen_t i = 0; i < feature.size(); ++i) {
      ...
      if (bin_idx < 0) bin_idx = 0;
      if (bin_idx >= bin_counts.size()) bin_idx = bin_counts.size() - 1;
...
#pragma omp atomic
  ...
  void mergeRareBins() {
    ...
    R_xlen_t total_count = feature.size();
    R_xlen_t n_bins = bin_counts.size();
    bool merged = true;
    
    while (merged && n_bins > min_bins) {
      merged = false;
      for (R_xlen_t i = 0; i < n_bins; ++i) {
        double bin_prop = static_cast<double>(bin_counts[i]) / total_count;
        if (bin_prop < bin_cutoff) {
          // Merge with adjacent bin with smallest count
          R_xlen_t merge_with = static_cast<R_xlen_t>(-1);
          if (i == 0) {
            merge_with = i + 1;
          } else if (i == n_bins - 1) {
            merge_with = i - 1;
          ...
          if (merge_with != static_cast<R_xlen_t>(-1)) {
            // Merge bins
  ...
  void mergeBins(R_xlen_t idx1, R_xlen_t idx2) {
    if (idx1 == idx2) return;
  ...
  void monotonicBinning() {
    ...
    while (!is_monotonic && bin_counts.size() > min_bins) {
      ...
      R_xlen_t idx_to_merge = findBinToMerge();
    ...
    while (bin_counts.size() > max_bins) {
      ...
      R_xlen_t idx_to_merge = findSmallestBin();
  ...
  void computeWOE() {
    ...
    if (total_pos == 0) total_pos = 1e-8;
    if (total_neg == 0) total_neg = 1e-8;
    
    woe_values.resize(bin_counts.size());
    for (R_xlen_t i = 0; i < bin_counts.size(); ++i) {
      double dist_pos = static_cast<double>(bin_count_pos[i]) / total_pos;
      double dist_neg = static_cast<double>(bin_count_neg[i]) / total_neg;
      // Assign small values to avoid log(0)
  ...
  void computeWOEIV() {
    ...
    woe_values.resize(bin_counts.size());
    iv_values.resize(bin_counts.size());
  ...
  bool checkMonotonicity() {
    ...
    for (R_xlen_t i = 1; i < woe_values.size(); ++i) {
  ...
  R_xlen_t findBinToMerge() {
    // Find the pair of bins with smallest difference in WOE
    double min_diff = std::numeric_limits<double>::infinity();
    R_xlen_t idx_to_merge = 0;
    for (R_xlen_t i = 0; i < woe_values.size() - 1; ++i) {
  ...
  R_xlen_t findSmallestBin() {
    // Find the bin with the smallest count
    R_xlen_t min_count = bin_counts[0];
    R_xlen_t idx = 0;
    for (R_xlen_t i = 1; i < bin_counts.size(); ++i) {
  ...
  R_xlen_t findBin(double value) const {
    // Find the bin index for a given value using binary search
    // bin_edges is sorted in ascending order
    // Returns index such that bin_edges[idx] < value <= bin_edges[idx + 1]
    // Since bin_edges includes -Inf and +Inf, upper_bound will always find a valid bin
    auto it = std::upper_bound(bin_edges.begin(), bin_edges.end(), value);
    return static_cast<R_xlen_t>(std::distance(bin_edges.begin(), it) - 1);
  }
  
  void updateBinLabels() {
    bin_labels.clear();
    R_xlen_t n_bins = bin_edges.size() - 1;
    for (R_xlen_t i = 0; i < n_bins; ++i) {
      bool is_first = (i == 0);
      bool is_last = (i == n_bins - 1);
      double lower = bin_edges[i];
      double upper = bin_edges[i + 1];
      std::string label = formatBinLabel(lower, upper, is_first, is_last);
      bin_labels.push_back(label);
    }
  }
  
  void enforceBinConstraints() {
    // Ensure that the number of bins is within [min_bins, max_bins]
    while (bin_counts.size() < min_bins) {
      // Merge the two bins with the smallest counts
      R_xlen_t idx1 = findSmallestBin();
      if (idx1 < bin_counts.size() - 1) {
        mergeBins(idx1, idx1 + 1);
      } else {
        mergeBins(idx1 - 1, idx1);
      }
      computeWOEIV();
    }
...
List optimal_binning_numerical_mba(IntegerVector target,
                                  NumericVector feature,
                                  R_xlen_t min_bins = 3, R_xlen_t max_bins = 5,
                                  double bin_cutoff = 0.05, R_xlen_t max_n_prebins = 20,
                                  int n_threads = 1) {
 // Handle edge cases where feature has only one unique value
 if (feature.size() == 0) {
   stop("Feature vector is empty.");
 }
 
 R_xlen_t unique_features = unique(feature).size();
 if (unique_features == 1) {
   // All values fall into a single bin
   std::vector<std::string> bin_labels = {"(-Inf;+Inf]"};
   std::vector<double> woe_values = {0.0}; // Assign WoE as 0 when no information
   std::vector<double> iv_values = {0.0};
   std::vector<R_xlen_t> bin_counts = {feature.size()};
   std::vector<R_xlen_t> bin_count_pos = {sum(target)};
   std::vector<R_xlen_t> bin_count_neg = {static_cast<R_xlen_t>(feature.size() - sum(target))};
   ...
   NumericVector woefeature(feature.size(), 0.0);
