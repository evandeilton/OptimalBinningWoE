...
public:
  ...
  OptimalBinningCategoricalCM(const std::vector<std::string>& feature,
    ...
    : feature(feature), target(target), min_bins(min_bins),
      ...
      unique_categories(0), is_increasing(true) {
  }
...
#pragma omp parallel
{
  std::unordered_map<std::string, int> local_total_count_map;
  std::unordered_map<std::string, int> local_count_pos_map;
  std::unordered_map<std::string, int> local_count_neg_map;
...
#pragma omp for nowait
  for (size_t i = 0; i < feature.size(); ++i) {
    ...
    local_total_count_map[cat]++;
    if (tar == 1) {
      local_count_pos_map[cat]++;
    } else {
      local_count_neg_map[cat]++;
    }
  }
  
#pragma omp critical
{
  for (const auto& item : local_total_count_map) {
    total_count_map[item.first] += item.second;
  }
  for (const auto& item : local_count_pos_map) {
    count_pos_map[item.first] += item.second;
  }
  for (const auto& item : local_count_neg_map) {
    count_neg_map[item.first] += item.second;
  }
}
}
...
#pragma omp parallel for reduction(min:min_chi_square)
...
#pragma omp parallel for
  ...
  // Enforce monotonicity of WoE (continued)
...
#pragma omp parallel for
    for (size_t i = 0; i < feature.size(); ++i) {
      ...
      if (it != category_to_woe.end()) {
        ...
        // This shouldn't occur as all categories should be in bins
        woefeature[i] = 0.0;
      }
    }
  }
  ...
  // Sort bins based on WoE
  void sort_bins_by_woe() {
    std::sort(bins.begin(), bins.end(),
              [this](const std::shared_ptr<Bin>& a, const std::shared_ptr<Bin>& b) {
                return is_increasing ? (a->woe < b->woe) : (a->woe > b->woe);
              });
  }
};
