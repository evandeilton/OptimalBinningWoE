...
void OptimalBinningCategoricalMBA::enforce_monotonicity() {
  if (bins.empty()) {
    Rcpp::stop("No bins available to enforce monotonicity.");
  }
  ...
  if (!check_monotonicity()) {
    Rcpp::warning("Monotonicity could not be achieved while respecting min_bins.");
  }
}
...
void OptimalBinningCategoricalMBA::optimize_bins() {
  ...
  if (bins.size() == static_cast<size_t>(max_bins) && !check_monotonicity()) {
    Rcpp::warning("Max bins reached but monotonicity not achieved.");
  }
}
...
void OptimalBinningCategoricalMBA::assign_woe() {
  ...
  const double default_woe = 0.0;
  
  for (size_t i = 0; i < feature.size(); ++i) {
    auto it = category_woe_map.find(feature[i]);
    woefeature[i] = (it != category_woe_map.end()) ? it->second : default_woe;
  }
}
...
void OptimalBinningCategoricalMBA::merge_bins(size_t index1, size_t index2) {
  if (index1 >= bins.size() || index2 >= bins.size() || index1 == index2) {
    Rcpp::stop("Invalid bin indices for merging.");
  }
  ...
  if (bins.empty()) {
    Rcpp::stop("All bins have been merged. Unable to continue.");
  }
}
...
void OptimalBinningCategoricalMBA::check_consistency() const {
  ...
  if (static_cast<size_t>(total_count) != feature.size()) {
    Rcpp::stop("Inconsistent total count after binning.");
  }
  
  if (total_count_pos != total_bad || total_count_neg != total_good) {
    Rcpp::stop("Inconsistent positive/negative counts after binning.");
  }
}
...
Rcpp::List OptimalBinningCategoricalMBA::fit() {
  ...
  return Rcpp::List::create(
    ...
    Rcpp::Named("is_monotonic") = check_monotonicity()
  );
}
...
Rcpp::List optimal_binning_categorical_mba(Rcpp::IntegerVector target,
 ...
 OptimalBinningCategoricalMBA mba(feature_vec, target, min_bins, max_bins, bin_cutoff, max_n_prebins);
